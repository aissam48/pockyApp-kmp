...
class ApiManager(val dataStore: DataStore<Preferences>) {
    ...
    suspend fun getBlockedUsers(
        ...
        try {
            val response: HttpResponse = client.get("$baseUrl/operations/blockedusers") {
                val token = getToken()
                contentType(ContentType.Application.Json)
                headers { append(HttpHeaders.Authorization, "Bearer $token") }
            }
    ...
    suspend fun getFollowingsMoments(
        onSuccess: (List<MomentModel>) -> Unit,
        onFailure: (ErrorModel) -> Unit
    ) {
        try {
            val response: HttpResponse = client.get("$baseUrl/operations/followings-moments") {
                val token = getToken()
                contentType(ContentType.Application.Json)
                headers { append(HttpHeaders.Authorization, "Bearer $token") }
            }
    ...
    suspend fun getFollowingsPosts(
        onSuccess: (List<PostModel>) -> Unit,
        onFailure: (ErrorModel) -> Unit
    ) {
        try {
            val response: HttpResponse = client.get("$baseUrl/operations/followings-posts") {
                val token = getToken()
                contentType(ContentType.Application.Json)
                headers { append(HttpHeaders.Authorization, "Bearer $token") }
            }
            ...
            if (response.status.isSuccess()) {
                val responseBody: List<PostModel> = response.body()
                println("success-----> ${response.bodyAsText()}")
                onSuccess(responseBody)
            } else {
                val errorMessage: ErrorModel = response.body()
